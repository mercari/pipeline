...
| rowFilter    | selective required | RowFilter      | Specify filter conditions for query from request to Bigtable                                                                                                                                                                                                                                                 |
| keyRanges    | selective required | KeyRange       | Specify key range conditions for queries from request to Bigtable                                                                                                                                                                                                                                            |
| columns      | optional           | Array<Column\> | Define the output schema by defining the mapping of each column to its type and feed name                                                                                                                                                                                                                    |
...
| field       | optional | String | Specify the name of the field that holds the retrieved column qualifier cell value                                                          |
| format      | optional | Enum   | Specify the cell value serialization format. The default is parent `format` value                                                           |
| cellType    | optional | Enum   | Specify cellType that defines how to retrieve the multiple cells associated with a column qualifier. The default is parent `cellType` value |
